...
misclassify.percent <-c(0.2, 0.4, 0.6)
...
for(count.con in 1:nrow(con)) { 
  ...
  for (r in 1:reps){
    ...
    for(p in 1:N.unmot){
      for (i in 1:I){
        ifelse(RG.matrix.unmot[p,i]==1,prob.unmot.NA[p,i]<-NA,prob.unmot.NA[p,i]<-IR.prob.unmot[p,i])
        ifelse(RG.matrix.unmot[p,i]==1,prob.unmot.zero[p,i]<-0,prob.unmot.zero[p,i]<-IR.prob.unmot[p,i])
    ...
    probs.combined.NA<-rbind(prob.unmot.NA,IR.prob.mot)
    probs.combined.zero<-rbind(prob.unmot.zero,IR.prob.mot)
    ...
    if(results.condition[r,28]==1){
      ...
      for(r.EM.impute in 1:reps.EM.imputed){
        ...
        if(extract.mirt(EM.imputed,'converged')==TRUE){ 
          ...
          a.EM.imputed[a.EM.imputed>3|a.EM.imputed<(-3)] <- NA
          b.EM.imputed[,r.EM.impute]<-ipars.EM.imputed[c(seq(2,200,4))] 
          b.EM.imputed[b.EM.imputed>3|b.EM.imputed<(-3)] <- NA
          #c.ML<-ipars.ML[c(seq(3,200,4))] 
...
#write.csv(overall.results.output,"40% Underclassifications 2022_07_28.csv",row.names = FALSE)
write.csv(overall.results.output,"Overall Results - Underclassification.csv",row.names = FALSE)
